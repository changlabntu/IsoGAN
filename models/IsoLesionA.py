...
class GAN(BaseModel):
    ...
    def backward_g(self):
        ...
        loss_g_l1Y = self.add_loss_l1(a=self.XupY[:, :, :, :, ::8], b=self.oriY[:, :, :, :, :]) * self.hparams.lamb

        loss_g = (loss_g_gan + loss_g_l1 + loss_g_l1Y) * 0.5

        return {'sum': loss_g, 'g_gan': loss_g_gan, 'g_l1': loss_g_l1, 'g_l1Y': loss_g_l1Y}

    def backward_d(self):
        loss_d = 0

        loss_d_gan = self.adv_loss_six_way(self.XupX, truth=False)
        loss_d_gan += self.adv_loss_six_way(self.XupY, truth=False)

        loss_d += loss_d_gan

        # ADV(X)+
        Xup = self.Xup.permute(4, 1, 2, 3, 0)[::1, :, :, :, 0]  # (Z, C, X, Y) # xy plane
        loss_d += self.add_loss_adv(a=Xup, net_d=self.net_d, truth=True) * 0.5
        Yup = self.Yup.permute(4, 1, 2, 3, 0)[::1, :, :, :, 0]
        loss_d += self.add_loss_adv(a=Yup, net_d=self.net_d, truth=True) * 0.5
