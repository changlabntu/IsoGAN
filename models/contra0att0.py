...
class GAN(BaseModel):
    def __init__(self, hparams, train_loader, eval_loader, checkpoints):
        ...
        # attention
        self.cross_attention = CrossAttention(self.hparams.ngf * 8)
        self.ca_proj = nn.Linear(2 * self.hparams.ngf * 8, self.hparams.ngf * 8)
        ...
        self.netd_names = {'net_d': 'net_d', 'classifier': 'classifier', 'projection': 'projection',
                           'cross_attention': 'cross_attention', 'ca_proj': 'ca_proj'}
    ...
    def backward_d(self):
        ...
        features_concat_proj = self.ca_proj(features_concat)

        # Use feature difference as query and concatenated features as key-value pairs
        output = self.cross_attention(feature_diff, feature_diff, feature_diff)
