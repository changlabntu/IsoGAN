...
class GAN(BaseModel):
    ...
    def generation(self, batch):
        ...
        # reshape from (B, 256, H, W, Z) to (B, 256)
    ...
    def backward_g(self):
        ...
        if self.hparams.adv > 0:
            ...
            # classification X vs XY IN ADVERSARIAL NEVER MIND
    ...
    def backward_d(self):
        ...
        output = self.classifier(torch.cat([fA, fB], dim=0))
        labels = torch.cat([torch.ones(fA.shape[0]), torch.zeros(fB.shape[0])])
        ...
        #imgYYcp = self.projection(self.imgYYc)  # -
        loss_center = self.center(torch.cat([f for f in [oriXcp, oriYcp, imgXYcp]], dim=0),
                                  torch.FloatTensor(2 * [1] * oriXcp.shape[0] + 1 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
        loss_tc, _ = self.tripletcenter(torch.cat([f for f in [oriXcp, oriYcp, imgXYcp]], dim=0),
                                        torch.FloatTensor(2 * [1] * oriXcp.shape[0] + 1 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
