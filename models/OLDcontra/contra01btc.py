...
class GAN(BaseModel):
    ...
    def generation(self, batch):
        ...
        if self.hparams.adv > 0:
            ...
            outX = self.net_gY(outXz, alpha=self.hparams.alpha, method='decode')
            self.imgXX = nn.Sigmoid()(outX['out1'])  # mask 0 - 1
            self.imgXX = combine(self.imgXX, self.oriX, method='mul')  # i am using masking (0-1) here

            outYz = self.net_g(self.oriY, alpha=self.hparams.alpha, method='encode')
            outY = self.net_gY(outYz, alpha=self.hparams.alpha, method='decode')
            self.imgYY = nn.Sigmoid()(outY['out1'])  # mask 0 - 1
        ...
        if self.hparams.adv > 0:
            ...
            self.imgXXc = self.net_d(self.imgXX)[1]
        ...
        if self.hparams.adv > 0:
            ...
            self.imgXXc = self.pool_3d_features(self.imgXXc, batch)
    ...
    def backward_g(self):
        ...
        if self.hparams.adv > 0:
            ...
            axx = self.add_loss_adv(a=self.imgXX, net_d=self.net_d, truth=True)
            ...
            loss_dict['loss_l1X'] = loss_l1X
            loss_g += loss_ga * self.hparams.adv + loss_l1Y * self.hparams.lamb + loss_l1X * self.hparams.lamb
    ...
    def backward_d(self):
        ...
        if self.hparams.adv > 0:
            ...
            # ADV(XY)-
            axx = self.add_loss_adv(a=self.imgXX.detach(), net_d=self.net_d, truth=False)
            # ADV(XX)-
            ax = self.add_loss_adv(a=self.oriX.detach(), net_d=self.net_d, truth=True)

            # adversarial of xy (-) and y (+)
            loss_da = axy * 0.25 + ay * 0.25 + axx * 0.25 + ax * 0.25 #+ ax * 0.25 + ay * 0.25
            # classify x (+) vs y (-)
        ...
        loss_center = self.center(torch.cat([f for f in [oriXcp, imgXXcp, oriYcp, imgYYcp]], dim=0),
                                  torch.FloatTensor(2 * [1] * oriXcp.shape[0] + 2 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
        loss_tc, _ = self.tripletcenter(torch.cat([f for f in [oriXcp, imgXXcp, oriYcp, imgYYcp]], dim=0),
                                        torch.FloatTensor(2 * [1] * oriXcp.shape[0] + 2 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
