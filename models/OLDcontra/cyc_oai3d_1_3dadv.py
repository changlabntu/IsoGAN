...
class GAN(BaseModel):
    ...
    def backward_g(self):
        ...
        loss_g_gan += self.add_loss_adv(a=self.imgYX.permute(4, 1, 2, 3, 0)[:, :, :, :, 0],  # (Z, C, X, Y)
                                       net_d=self.net_d, truth=True)
        loss_g_gan += self.add_loss_adv(a=self.imgYX.permute(4, 1, 3, 2, 0)[:, :, :, :, 0],  # (Z, C, Y, X)
                                       net_d=self.net_d, truth=True)
        loss_g_gan = loss_g_gan / 6
    ...
    def backward_d(self):
        ...
        loss_d_gan += self.add_loss_adv(a=self.imgYX.permute(4, 1, 2, 3, 0)[:, :, :, :, 0],  # (Z, C, X, Y)
                                       net_d=self.net_d, truth=False)
        loss_d_gan += self.add_loss_adv(a=self.imgYX.permute(4, 1, 3, 2, 0)[:, :, :, :, 0],  # (Z, C, Y, X)
                                       net_d=self.net_d, truth=False)
        loss_d_gan = loss_d_gan / 6
        loss_d += loss_d_gan
