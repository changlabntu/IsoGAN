...
class GAN(BaseModel):
    ...
    def backward_d(self):
        ...
        # classification by difference X vs XY and Y vs YY
        #dA = self.oriXc - self.imgXYc
        #dB = self.oriYc - self.imgYYc
        time_factor = 1 - self.epoch / self.hparams.n_epochs
        dA = self.oriXc - (time_factor * self.oriYc + (1 - time_factor) * self.imgXYc)
        dB = self.oriYc - (time_factor * self.oriXc + (1 - time_factor) * self.imgYYc)
        output = self.classifier(torch.cat([dA, dB], dim=0))
        labels = torch.cat([torch.FloatTensor(dA.shape[0] * [1]).type(torch.LongTensor),
                            torch.FloatTensor(dB.shape[0] * [0]).type(torch.LongTensor)], dim=0).cuda()
        ...
        loss_center = self.center(torch.cat([f for f in [oriXcp, oriYcp, imgYYcp]], dim=0),
                                  torch.FloatTensor(1 * [1] * oriXcp.shape[0] + 2 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
        loss_tc, _ = self.tripletcenter(torch.cat([f for f in [oriXcp, oriYcp, imgYYcp]], dim=0),
                                        torch.FloatTensor(1 * [1] * oriXcp.shape[0] + 2 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
