...
class GAN(BaseModel):
    ...
    def __init__(self, hparams, train_loader, eval_loader, checkpoints):
        ...
        self.hparams.final = 'tanh'
        _, self.net_dm = self.set_networks()

        # save model names
        self.netg_names = {'net_g': 'net_g', 'net_gback': 'net_gback'}
        self.netd_names = {'net_d': 'net_d', 'net_dzy': 'net_dzy', 'net_dzx': 'net_dzx', 'net_dm': 'net_dm'}
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--lbM', type=float, default=1.0, help='weight for masking')
    ...
    def generation(self, batch):
        ...
        if self.hparams.transpose:
            ...
            self.Mup = self.Mup.permute(0, 1, 3, 4, 2)  # (B, C, Y, Z, X)

        self.goutz = self.net_g(self.Xup, method='encode')
        out = self.net_g(self.goutz[-1], method='decode')
        self.XupX = out['out0']  # -1 ~ 1
        self.MupM = out['out1']  # -1 ~ 1

        # masking XupX by MupM
        mask_to_01 = torch.multiply((self.XupX + 1) / 2, (self.MupM + 1) / 2)
        self.XupX = (mask_to_01 - 0.5) * 2 * 0.5 + self.XupX * 0.5
    ...
    def backward_g(self):
        ...
        if self.hparams.lbM > 0:
            amm = self.adv_loss_six_way(self.MupM, net_d=self.net_dm, truth=True)
            m_L1 = self.add_loss_l1(a=self.MupM[:, :, :, :, ::self.uprate * self.hparams.skipl1],
                                    b=self.Mup[:, :, :, :, ::self.uprate * self.hparams.skipl1]) * self.hparams.lbM
            loss_dict['amm'] = amm
            loss_g += amm
            loss_dict['m_L1'] = m_L1
            loss_g += m_L1
    ...
    def backward_d(self):
        ...
        if self.hparams.lbM > 0:
            dmm = self.adv_loss_six_way(self.MupM, net_d=self.net_dm, truth=False)
            # ADV(M)+
            dm = self.add_loss_adv(a=self.get_xy_plane(self.oriM), net_d=self.net_dm, truth=True)
            loss_dict['dmm'] = dmm + dm
            loss_d += dmm + dm
