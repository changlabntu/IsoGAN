...
class GAN(BaseModel):
    ...
    def backward_d(self):
        ...
        imgXYcp = self.projection(self.imgXYc)  # -
        imgYYcp = self.projection(self.imgYYc)  # -
        loss_center = self.center(torch.cat([f for f in [oriXcp, oriYcp, imgXYcp, imgYYcp]], dim=0),
                                  torch.FloatTensor(1 * [1] * oriXcp.shape[0] + 3 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
        loss_tc, _ = self.tripletcenter(torch.cat([f for f in [oriXcp, oriYcp, imgXYcp, imgYYcp]], dim=0),
                                        torch.FloatTensor(1 * [1] * oriXcp.shape[0] + 3 * [0] * oriYcp.shape[0]).type(
                                            torch.LongTensor).cuda())
